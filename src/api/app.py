...
best_scikit_learn_onnx = rt.InferenceSession("src/models/best_scikit_learn.onnx")
best_scikit_learn_input_name = best_scikit_learn_onnx.get_inputs()[0].name
...
def predict(input_data: InputData):
    ...
    transformed_categorical = one_hot_encoder_onnx.run(
        None, {one_hot_encoder_input_name: categorical_array}
    )
    transformed_categorical = np.array(transformed_categorical[0]).reshape(
        transformed_categorical[0].shape[0], -1
    )
    # sandard conversion
    continuous_array = np.array(data_array[:, continuous_columns], dtype=np.int64)
    transformed_continuous = standard_scaler_onnx.run(
        None, {standard_scaler_input_name: continuous_array}
    )
    transformed_continuous = np.array(transformed_continuous[0]).reshape(
        transformed_continuous[0].shape[0], -1
    )
    ...
    predictions = best_scikit_learn_onnx.run(
        None, {best_scikit_learn_input_name: data_array}
    )
